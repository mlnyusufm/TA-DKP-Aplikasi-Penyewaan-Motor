...
class ReturnMotorFrame(Page):
    def __init__(self, master):
        super().__init__(master)
        self.master = master

        self.labelReturnMotor = ctk.CTkLabel(self, text="Tambah List Pengembalian Motor", font=("Arial Bold", 20))
        ...
        self.buttonAddReturn = ctk.CTkButton(self, text="Tambah Pengembalian", command=self.addReturn)
        self.buttonAddReturn.pack(pady=10, padx=10)
    ...
    def addReturn(self):
        ...
        if plat not in self.master.stockMotor[brand][model]:
            messagebox.showerror("Error", "Nomor Plat motor yang diminta tidak tersedia.")
            return

        self.master.stockMotor[brand][model][plat] += 1
        self.master.borrowingHistory.append(("Pengembalian", brand, model, plat))

        messagebox.showinfo("Sukses", f"Pengembalian motor {brand} {model} Nomor Plat {plat} berhasil ditambahkan.")
        self.clearFields()
